...
const bgImages = [
    "/heroBackround/1.webp",
    "/heroBackround/2.webp",
    "/heroBackround/3.webp",
    "/heroBackround/4.webp",
    "/heroBackround/5.webp",
    "/heroBackround/6.webp",
];

const platforms = [
    { name: "Google", logo: "/logo/1.webp" },
    { name: "ChatGPT", logo: "/logo/2.webp" },
    { name: "Gemini", logo: "/logo/3.webp" },
    { name: "TikTok", logo: "/logo/4.webp" },
    { name: "YouTube", logo: "/logo/5.webp" },
    { name: "Pinterest", logo: "/logo/6.webp" },
    { name: "GIPHY", logo: "/logo/7.webp" },
    { name: "reddit", logo: "/logo/8.webp" },
    { name: "amazon", logo: "/logo/9.webp" },
];
...
export default function HeroSection({ isBlurred }: HeroProps) {
    const [bgImage] = useState<string>(
        () => bgImages[Math.floor(Math.random() * bgImages.length)]
    );

    return (
        <div className="w-full px-2 pb-2 h-screen" style={{ filter: isBlurred ? "blur(8px)" : "blur(0px)" }}>
            <div className="relative w-full h-full rounded-3xl overflow-hidden">
                ...
                >
                    <div className="absolute inset-0 flex flex-col items-center justify-center text-center px-4">
                        <p className="text-white text-[13px] font-medium tracking-tighter uppercase leading-3.5 mb-3">
                        ...
                        <div className="flex items-center justify-center gap-2 md:gap-4 mb-6 md:mb-8">
                            ...
                            <Image src="/hero/globalSearch.webp" alt="Global Search Awards" width={50} height={30} className="object-contain brightness-0 invert" />
                            <Image src="/hero/Mask-group.webp" alt="The Drum" width={30} height={30} className="object-contain brightness-0 invert" />
                            <Image src="/hero/UKSocial.webp" alt="UK Social" width={50} height={40} className="object-contain brightness-0 invert" />
                            <Image src="/hero/UKContent.webp" alt="UK Content" width={50} height={30} className="object-contain brightness-0 invert lg:flex md:hidden hidden" />
                            <svg viewBox="0 0 28 38" fill="none" xmlns="http://www.w3.org/2000/svg" className="w-6 fill-current text-white">
                        ...
                        <h1
                            className="text-white font-medium lg:leading-[0.8] leading-14 tracking-[-0.04em] mb-6 lg:mb-8 md:mb-2 text-6xl md:text-6xl lg:text-9xl">
                            We Create<br />
                            <span className="flex leading-8 items-center justify-center gap-3 flex-wrap lg:leading-none md:leading-none">
                                Category
                                <span
                                    className="relative inline-block lg:rounded-2xl md:rounded-xl rounded-lg mt-1 md:mt-2 lg:mt-2 overflow-hidden shrink-0"
                                    style={{ width: "clamp(55px, 7vw, 120px)", height: "clamp(55px, 7vw, 120px)" }}
                                >
                        ...
                        <p className="text-white text-lg md:text-[22px] lg:text-3xl font-semibold tracking-tighter mb-14">
                            on every searchable platform
                        </p>

                        <div className="hidden lg:flex flex-wrap items-center justify-center gap-x-10 gap-y-6 xl:gap-x-16">
                            {platforms.map((p) => (
                                <Image
                                    key={p.name}
                                    src={p.logo}
                    ...
                    <div className="absolute hidden md:flex lg:flex text-left left-5 bottom-4">
                        <p className="text-white md:text-[14px] lg:text-[17px] text-[16px] leading-relaxed tracking-tight">
